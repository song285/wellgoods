<template>
	<view class="main">
		...
		<view class="details">
			<image v-for="img in detailImg" :key="item" :src="img.url" mode="widthFix"></image>
		</view>
	...
	export default {
		...
		methods: {
			goBack() { //后退
				...
				if(this.type || (this.$store.state.invitationCode && this.goodsId)){
					uni.reLaunch({
						url: "./index"
					})
					return
			...
			showService() { //客服弹窗
				if (this.$store.state.userToken == '' || this.$store.state.userToken == undefined) { //判断是否登录
					this.$store.commit("showBindingPhone")
					return
				}
				this.isService = true
		...
		onLoad(opstions) {
			...
			// 通过分享进入
			if (opstions.inviteCode && opstions.goodsId) {
				this.$store.commit("invitation", opstions.inviteCode)
