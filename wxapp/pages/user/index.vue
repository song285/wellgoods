		...
		<view class="mian">
			...
			<view class="top">
				...
				<view class="save-money" @click="goSingleCardFree">
					<view>零点好物已为你省了{{userData.totalSaveAmt || 0}}元</view>
			...
			<view class="my-deal fillet">
				...
				<view class="state">
					<view class="item" @click="goMyOrder(2)">
						...
						<view>待完成</view>
						<view class="number" v-if="myOrder.UN_FINISH.length > 0">{{myOrder.UN_FINISH.length > 99 ? '99+' : myOrder.UN_FINISH.length}}</view>
					</view>
					<view class="item" @click="goMyOrder(3)">
						<image src="http://47.111.232.251:8106/path?name=1605065593824.png" mode=""></image>
						<view>待收货</view>
						<view class="number" v-if="myOrder.TO_GET_GOODS.length > 0">{{myOrder.TO_GET_GOODS > 99 ? '99+' : myOrder.TO_GET_GOODS.length}}</view>
					</view>
					...
					<view class="item" @click="navigateToEva">
						...
						<view>邀请评价</view>
						<view class="number" v-if="userData.evaluateNumber > 0">{{userData.evaluateNumber > 99 ? '99+' : userData.evaluateNumber}}</view>
					</view>
				</view>
			</view>
			<!-- 我的成就 -->
