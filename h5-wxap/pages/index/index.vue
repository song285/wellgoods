		...
		<view class="main">
			...
			<view class="content">
				...
				<scroll-view scroll-y="true" class="scroll-class">
					...
					<!-- <swiper :duration="1000" class="articles-type" :class="ifFixedType ? 'fixed-type' :''">
						<swiper-item>
							<view :class="choiceIndex==-1 ? 'on' :'' " @tap="choice(-1,'')">今日热门</view>
						</swiper-item>
						<swiper-item 
					...
					<view class="commodity">
						<view class="item" v-for="item in productList" :key="item.mainPic" @tap="goDetails(item)">
	...
	export default {
		data() {
			return {
				...
				articles:[],//分类
				choiceIndex:-1,//分类选中颜色
				ifFixedType:false,//判断类型是否固定头部
				productList:[],//商品数据
				zoneList:{
					zeroStatus:0,
					ninePointNineStatus:0
				},//专区商品
				searchText:'',
				typeId:'',
				isSensitive:0,
		...
		onLoad(opstion) {
			uni.setNavigationBarTitle({
				title:this.$store.state.navigationBarTitle || '首页'
			})
			// 开发状态  上线时删除该行
			// let url = "http://wx.haichennux.net/index.htm?openid=oF9G51eD-9TQzFtYSYU8RR4E8SP4&substationId=1346651624541655040&inviteCode=#/pages/user/myOrder?state=4"
			let url = ''
			// 解析网址参数
			this.getQueryObject(url)
			
			common.author4H5(this)
			
			// #ifdef MP-WEIXIN
				uni.hideHomeButton() //隐藏左上角小房子
			// #endif
			this.$store.commit("invitation",this.$store.state.userBaseinfo.inviteCode)
...
<style scoped>
	.scroll-class {
		max-height: calc(100vh - 44px - 28px - 48px);
	}
	.main {
